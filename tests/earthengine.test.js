const { OpenEO, Connection, FileTypes, Capabilities, UserProcess, Job, Service, File, BasicProvider } = require('../openeo.node.js');
...
//	const TESTBACKEND = 'http://127.0.0.1:8080';
	...
	const PROCESSGRAPH = {"1":{"process_id":"load_collection","arguments":{"id":"COPERNICUS/S2","spatial_extent":{"west":-2.763447,"south":43.040791,"east":-1.120991,"north":43.838489},"temporal_extent":["2018-04-30","2018-06-26"],"bands":["B4","B8"]}},"2":{"process_id":"normalized_difference_fast","arguments":{"data":{"from_node":1},"band1":"B4","band2":"B8"}},"3":{"process_id":"reduce","arguments":{"data":{"from_node":2},"reducer":{"process_graph":{"min":{"arguments":{"data":{"from_parameter":"data"}},"process_id":"min","result":true}}},"dimension":"t"}},"4":{"process_id":"apply","arguments":{"data":{"from_node":3},"process":{"process_graph":{"lsr":{"arguments":{"x":{"from_parameter":"x"},"inputMin":-1,"inputMax":1,"outputMin":0,"outputMax":255},"process_id":"linear_scale_range","result":true}}}}},"5":{"process_id":"save_result","arguments":{"data":{"from_node":4},"format":"png"},"result":true}};
	const VALID_PROCESS = {process_graph: PROCESSGRAPH};
	const INVALID_PROCESS = {"load": {"process_id": "load_collection","arguments": {}}};

	const TESTFORMATS = {"input":{},"output":{"PNG":{"title":"PNG","gis_data_types":["raster"],"parameters":{"red":{"type":"string","format":"band-name","description":"Band name being used for the red channel.","default":null},"green":{"type":"string","format":"band-name","description":"Band name being used for the green channel.","default":null},"blue":{"type":"string","format":"band-name","description":"Band name being used for the blue channel.","default":null},"gray":{"type":"string","format":"band-name","description":"Band name being used as a gray channel.","default":null}}},"JPEG":{"title":"JPG / JPEG","gis_data_types":["raster"],"parameters":{"red":{"type":"string","format":"band-name","description":"Band name being used for the red channel.","default":null},"green":{"type":"string","format":"band-name","description":"Band name being used for the green channel.","default":null},"blue":{"type":"string","format":"band-name","description":"Band name being used for the blue channel.","default":null},"gray":{"type":"string","format":"band-name","description":"Band name being used as a gray channel.","default":null}}},"JSON":{"gis_data_types":["raster","vector","table","other"]}}};
	...
	describe('Discovery', () => {
		...
		test('File types', async () => {
			...
			expect(types.toPlainObject()).toEqual(TESTFORMATS);
			expect(Utils.size(types.getInputTypes())).toBe(0);
			expect(Utils.size(types.getOutputTypes())).toBe(Object.keys(TESTFORMATS.output).length);
			expect(types.getInputType('PNG')).toBeNull();
			expect(types.getOutputType('PNG')).toEqual(TESTFORMATS.output.PNG);
			expect(types.getOutputType('png')).toEqual(TESTFORMATS.output.PNG);
		});
